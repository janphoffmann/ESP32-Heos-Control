...
bool DENON_AVR::begin(const char* FriendlyName, int port) {
...
void DenonProperty::_feedback(const char *data, size_t len) {
  String in(data);
...
void DenonProperty::addCommand(const char *Command) {
...
// DenonProperty::DenonProperty(DENON_AVR *i, int j) {
//   obj = i;
//   obj->no[j] = j;
//   objNo = j;
// }

int DenonProperty::_objNo = 0;
DENON_AVR* DenonProperty::obj = NULL;

DenonProperty::DenonProperty(DENON_AVR* i){
  obj = i;
}

DenonProperty::DenonProperty(const char * Command, int j){
  obj->no[j] = j;
  objNo = j;
  addCommand(Command);
  _objNo++;
...
DenonProperty::DenonProperty(DENON_AVR* i, const char * Command, int j){
  obj = i;
  obj->no[j] = j;
  objNo = j;
  addCommand(Command);
...
void DenonProperty::set(const char *i) {
  obj->set(_Command, i);
  strncpy(is, i,sizeof(is));
  // is = (char *)i;
}

char *DenonProperty::get() {
  return is;
}
int DenonProperty::get(int i){
  String j(is);
  if(_state_update_int_cb != 0) return j.toInt();
  return -1;  
}

void DenonProperty::onStateUpdate(ResponseHandler callbackFunc){
  _state_update_cb = callbackFunc;
}

void DenonProperty::onStateUpdate(ResponseHandlerInt callbackFunc){
